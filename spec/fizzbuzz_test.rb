...
class FizzBuzzTest < Minitest::Test
  def test_fizzbuzz_exists #must start method with "test otherwise minitest won't work"
    assert(Fizzbuzz)
  end

  def test_run_exists
    fb = Fizzbuzz.new
    assert_respond_to(fb, :run) #does fizzbuzz class have a method called run
  end
  ...
  def test_fizz_buzz_fizzbuzz_returns_strings
    fb = Fizzbuzz.new
  ...
  def test_buzz_returns_given_number
    fb = Fizzbuzz.new
    assert_equal(6, fb.buzz(6))
  end

  def test_fizzbuzz_returns_for_div_by_3_and_5_numbers
    fb = Fizzbuzz.new
    assert_equal("fizzbuzz", fb.fizzbuzz(15))
  end

  def test_fizzbuzz_returns_given_number
    fb = Fizzbuzz.new
    assert_equal("fizzbuzz", fb.fizzbuzz(30))
  end
